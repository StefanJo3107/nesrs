...
fn show_tile(chr_rom: &Vec<u8>, bank: usize, tile_n: usize) -> Frame {
    ...
    // get one of two banks (second bank starts at 0x1000)
    let bank = (bank * 0x1000) as usize;

    // get tile (each tile is 8x8 bits times 2 for two bit palette value
    let tile = &chr_rom[(bank + tile_n * 16)..=(bank + tile_n * 16 + 15)];

    for y in 0..=7 {
        // upper byte for palette index
        let mut upper = tile[y];
        // lower byte for palette index
        let mut lower = tile[y + 8];

        for x in (0..=7).rev() {
            // extract two bits for each pixels palette index
